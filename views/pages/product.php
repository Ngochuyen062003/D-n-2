        ...
        <div class="md:w-7/12 md:pl-11">
            <div class="mb-4">
                ...
                <a class="hover:text-primary"
                    href="<?php echo get_home_url() . 'category?id=' . $product['category'][0]['id'] ?>"><?php echo $product['category'][0]['name'] ?></a>
            </div>
            ...
            <form class="add-to-cart" method="POST" action="<?php echo get_home_url().'/cart' ?>">
                ...
                <input type="text" name="item[0][id]" hidden value="<?php echo $product['id'] ?>">
                <div class="flex items-end mb-4">
                    <p class="w-16 font-semibold">Color:</p>
                    <div class="flex ml-6">
                        <?php foreach ($product['color'] as $key => $value) : ?>
                        <div class="flex mr-2 last:mr-0">
                            <input <?php if ($key === 0) { echo 'checked'; } ?> id="color-radio-<?php echo $value['id'] ?>" type="radio" value="<?php echo $value['id'] ?>" name="item[0][color]" class="peer" hidden>
                ...
                <div class="flex items-end mb-4">
                    ...
                    <div class="flex ml-6">
                        ...
                        <div class="flex mr-2 last:mr-0">
                            <input <?php if ($key === 0) { echo 'checked'; } ?> id="size-radio-<?php echo $value['id'] ?>" type="radio" value="<?php echo $value['id'] ?>" name="item[0][size]" class="peer" hidden>
                ...
                <div class="flex">
                    <div class="p-1 quantity-input shadow-default rounded w-fit flex items-center">
                        ...
                        <input data-input="quantity" name="item[0][quantity]" type="number" class="w-7 h-7 text-center mx-2 outline-none focus:outline-none focus:border-primary border-b-2 duration-150" min="1" value="1">
